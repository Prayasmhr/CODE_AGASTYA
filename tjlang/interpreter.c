...
void _interpreter_init(const void* ir,void** interpret_process_state) {
    *interpret_process_state = malloc(sizeof(tjlang_interpret_state));
    ...
    _interpreter_type_fntable_insert(interpret_state,"tj_label",_interpreter_interpret_label);
}
...
static bool _interpreter_interpret_button(tjlang_interpret_state* interpret_state,tjlang_ir_zone* ir_zone,tjlang_ir_instruction button_def,tj_uientity** uibutton) {
    ...
    if(strncmp(button_def.data.set_uientt_prop.prop.data,"text_buffer",12) == 0 && button_def.data.set_uientt_prop.data_type == UIENTT_PROP_DATA_TYPE_STRING) {
        ...
        if (FG){
            button->fg = UINT_ARRAY_TO_COLOR(array_instruction);
        }else if(BG) {
            button->bg = UINT_ARRAY_TO_COLOR(array_instruction); 
        }
        exit_code = 1;
...
static bool _interpreter_interpret_label(tjlang_interpret_state* interpret_state,tjlang_ir_zone* ir_zone,tjlang_ir_instruction label_def,tj_uientity** label) { 
    if(*label == NULL) {
        *label = (tj_uientity*)new_tj_label(new_string(""),(tj_color) {0,0,0,0}, (tj_color) {0,0,0,0,}, NULL, NULL);
    }
    tj_label uilabel = (tj_label)(*label);
    bool FG = strncmp(label_def.data.set_uientt_prop.prop.data,"fg",2) == 0;
    bool BG = strncmp(label_def.data.set_uientt_prop.prop.data,"bg",2) == 0;
    bool exit_code = 0;

    if(strncmp(label_def.data.set_uientt_prop.prop.data,"text_buffer",12) == 0 && label_def.data.set_uientt_prop.data_type == UIENTT_PROP_DATA_TYPE_STRING) {
        uilabel->text_buffer = new_string((char*)label_def.data.set_uientt_prop.value);
        exit_code = 1;
    }else if(FG || BG) {
        CHECK_ARRAY(label_def,BASIC_LANG_TYPE_UINTEGER,4);
        if (FG){
            uilabel->fg = UINT_ARRAY_TO_COLOR(array_instruction);
        }else if(BG) {
            uilabel->bg = UINT_ARRAY_TO_COLOR(array_instruction); 
        }
        exit_code = 1;
    }else if(strncmp(label_def.data.set_uientt_prop.prop.data,"font",5) == 0) {
        if(label_def.data.set_uientt_prop.data_type != UIENTT_PROP_DATA_TYPE_IDENTIFIER)
            printf("expected a identifier\n");
        char* f_asset_tag = (char*)label_def.data.set_uientt_prop.value;
        tj_asset* f_asset = asset_btree_search_by_tag(f_asset_tag);
        uilabel->font = f_asset->asset.font_handle;
        exit_code = 1;
    }else if(strncmp(label_def.data.set_uientt_prop.prop.data,"image",6) == 0) {
        if(label_def.data.set_uientt_prop.data_type != UIENTT_PROP_DATA_TYPE_IDENTIFIER)
            printf("expected a identifier\n");
        char* img_asset_tag = (char*)label_def.data.set_uientt_prop.value;
        tj_asset* img_asset = asset_btree_search_by_tag(img_asset_tag);
        uilabel->image = img_asset->asset.img_handle;
        exit_code = 1;
    }
    *label = (tj_uientity*)uilabel;
    return exit_code;
}
