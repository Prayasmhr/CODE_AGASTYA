...
bool agy_app_init(tj_appstate* self) {
    ...
    tjlang_interpret("agy_app","agy_app/scripts/main_mode.tjui","agy_app/scripts/app_script.c");

    self->inst->message = new_nstring("Hello", 6);
    self->inst->active_ui_env = tj_uienv_tree_get("main_mode_env");
...
i32 agy_app_main(tj_appstate *app_state)
{
    ...
    while (app_state->running) { 
        ...
        render_engine.clear((tj_color) { .R = 25, .G = 25,. B = 25,.A = 255});
